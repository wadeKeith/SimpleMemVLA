...
class Robot:
        ...
        if self.is_dual_arm:
            ...
            for i, joint in enumerate(self.right_active_joints):
                if joint not in self.right_gripper:
                    joint.set_drive_property(stiffness=self.right_joint_stiffness, damping=self.right_joint_damping,)
            for joint in self.right_gripper:
                joint[0].set_drive_property(stiffness=self.right_gripper_stiffness, damping=self.right_gripper_damping,)
    ...
    def print_info(self):
        if self.is_dual_arm:
            ...
            print(
                "active joints: ",
                [joint.get_name() for joint in self.left_active_joints],
            )
            print(
                "all links: ",
                [link.get_name() for link in self.left_entity.get_links()],
            )
            print("arm joints: ", [joint.get_name() for joint in self.left_arm_joints])
    ...
    def set_planner(self, scene=None):
        ...
        # [memVLA-RL patch] Force in-process CuRobo planners: never spawn per-arm mp.Process planner
        # subprocesses. The spawn path (torch.multiprocessing spawn) crashes under Ray actors / thread
        # pools at multi-GPU scale, and is unnecessary for RL rollout, which emits qpos actions executed
        # via mplib TOPP (CPU) rather than CuRobo. Matches SimpleVLA-RL / DeepThinkVLA_robotwin2.
    ...
    def get_normal_real_gripper_val(self):
        if not self.is_dual_arm:
            ...
            return [normal_left_gripper_val]
        else:
            normal_left_gripper_val = (self.left_gripper[0][0].get_drive_target()[0] - self.left_gripper_scale[0]) / (
                self.left_gripper_scale[1] - self.left_gripper_scale[0])
