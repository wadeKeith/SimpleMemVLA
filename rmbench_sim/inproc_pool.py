...
from rmbench_sim.rmbench_env import SimEnvService
...
class InProcSimPool:
    ...
    def start(self):
        self.services = [
            SimEnvService(self.task_config, self.repo_root, self.instruction_type,
                          dense_substeps=self.dense_substeps)
    ...
    def step(self, action_chunks: list, active: list[int],
             capture_stride: int = 1, frame_index: list | None = None) -> list[dict]:
        results: list = [None] * self.num_envs
        futs = {}
        for i in active:
            if not self.alive[i]:
                continue
            futs[self._pool.submit(self.services[i].step, {
                "action_chunk": action_chunks[i],
                "capture_stride": capture_stride,
                "frame_index": 0 if frame_index is None else int(frame_index[i]),
            })] = i
        for fut, i in futs.items():
            try:
