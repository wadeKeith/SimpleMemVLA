...
from simplememvla.benchmarks.rmbench import NUM_ACTIONS_CHUNK


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--pretrained_checkpoint", required=True)
    parser.add_argument("--root", default="./data/datasets/yinchenghust/rmbench_lerobot")
    parser.add_argument("--repo_id", default="yinchenghust/rmbench_lerobot")
    ...
    parser.add_argument(
        ...
        help="Re-predict every N frames (receding horizon). Defaults to 16 to match the "
             "closed-loop eval's --execute_horizon, so this open-loop score is a faithful "
             "quality proxy for closed loop (same re-decision cadence).",
    )
...
def main() -> None:
    ...
    for ep in episodes:
        ...
        for i in range(len(ds)):
            ...
            ep_hit += int(_norm(gen_subtask) == _norm(item["subtask"]))
            ep_pred += 1
