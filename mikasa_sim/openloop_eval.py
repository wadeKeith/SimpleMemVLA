...
from simplememvla.benchmarks.mikasa import NUM_ACTIONS_CHUNK


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--pretrained_checkpoint", required=True)
    parser.add_argument("--root", default="./data/datasets/yinchenghust/mikasa_lerobot")
    parser.add_argument("--repo_id", default="yinchenghust/mikasa_lerobot")
    ...
    parser.add_argument(
        ...
        help="Re-predict every N frames (receding horizon). Defaults to 8 to "
...
def main() -> None:
    ...
    if meta.subtasks is None:
        raise RuntimeError("Dataset has no meta/subtasks.parquet")
    _subtask_by_index = {
        int(v): s for s, v in meta.subtasks["subtask_index"].items()
    }

    def gt_subtask(item):
        return _subtask_by_index[int(item["subtask_index"].reshape(-1)[0])]
    ...
    print("=" * 60)
    print(f"Overall action_L1 : {tot_l1 / max(tot_steps, 1):.4f} (raw action space)")
