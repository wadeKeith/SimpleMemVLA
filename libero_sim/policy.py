...
from simplememvla.benchmarks.libero import CAMERA_LABELS
...
def get_vla(
    ...
    _VERIFIED_EQUIVALENT = {frozenset(("5.11.0", "5.13.1"))}
    saved_tf = getattr(vla.config, "transformers_version", None)
    if saved_tf is not None and saved_tf != _tf.__version__:
        if frozenset((saved_tf, _tf.__version__)) in _VERIFIED_EQUIVALENT:
            print(
                f"[policy] note: checkpoint saved with transformers=={saved_tf}, "
                f"env runs {_tf.__version__} — this pair was verified "
                "bit-identical on the closed-loop path.",
                flush=True,
            )
        else:
            print(
...
class LiberoPolicy:
    ...
    def _prepare_inputs(self, prompt: str) -> dict:
        ...
        oldest: dict = {}
        for k in keys:
            oldest[k[0]] = min(oldest.get(k[0], k[1]), k[1])
        for k in [k for k in self._patch_feat_cache if k[1] < oldest.get(k[0], 0)]:
    ...
    def predict(
        ...
        if self.use_proprio:
            if state is None:
                raise ValueError(
                    ...
                    "requires the current `state` (joint_pos + gripper_qpos)."
                )
            ...
            if state_arr.ndim != 1 or state_arr.shape[0] != expected:
                raise ValueError(
                    ...
                    f"[joint_pos(7) rad, gripper_qpos(2)], got shape "
                    f"{state_arr.shape}. The closed-loop state must match the dataset's "
                    "observation.state (same order/units)."
                )
        ...
        if int(full_ids[0, -1]) != self.im_end_id:
            print(
                ...
                f"{self.max_reasoning_tokens}-token budget without <|im_end|>; "
