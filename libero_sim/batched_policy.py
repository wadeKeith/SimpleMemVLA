...
class BatchedEvalPolicy:
    ...
    def generate_batch(
        ...
        first_logits = lm_head(out.last_hidden_state[:, -1]).float()
        if self.suppress_token_ids:
            first_logits[:, self.suppress_token_ids] = float("-inf")
        next_tok = first_logits.argmax(-1)
