...
"""
Oracle motion planner for TraceShape procedural-memory tasks.
...
Examples:
python mikasa_robo_suite/vla/utils/motion_planning/motion_planning_trace_shape.py --env-id TraceShapeEasy-VLA-v0 --seed 123 --save-video 0
python mikasa_robo_suite/vla/utils/motion_planning/motion_planning_trace_shape.py --env-id TraceShapeMedium-VLA-v0 --seed 7 --save-video 0
python mikasa_robo_suite/vla/utils/motion_planning/motion_planning_trace_shape.py --env-id TraceShapeHard-VLA-v0 --seed 42 --save-video 1
"""

DEFAULT_ENV_ID = "TraceShapeHard-VLA-v0"
VALID_ENV_IDS = {
    "TraceShapeEasy-VLA-v0",
    "TraceShapeMedium-VLA-v0",
    "TraceShapeHard-VLA-v0",
...
def main():
    ...
    shape_id = _to_int_scalar(env_u.shape_type[0]) if hasattr(env_u, "shape_type") else -1
    checkpoints = env_u.checkpoints[0].detach().cpu().numpy()
    waypoints = env_u.waypoints[0].detach().cpu().numpy()
    ...
    for _ in range(int(args.max_checkpoint_passes)):
        ...
        visited = env_u.checkpoint_visited[0].detach().cpu().numpy().astype(bool)
    ...
    info_now = env_u.evaluate()
    if (not episode_done["value"]) and (not _to_bool_scalar(info_now.get("success", False))):
        n_wp = len(waypoints)
        idxs = list(range(0, n_wp, int(args.trace_waypoint_stride)))
        if len(idxs) == 0 or idxs[-1] != n_wp - 1:
            idxs.append(n_wp - 1)
        # Explicitly close the loop by returning to waypoint 0.
        if idxs[-1] != 0:
            idxs.append(0)
    ...
    visited_final = env_u.checkpoint_visited[0].detach().cpu().numpy().astype(bool)
    visited_count = int(visited_final.sum())
    total_count = int(visited_final.shape[0])

    print("env_id:", env_id)
    print("shape_id:", int(shape_id))
    print("shape_name:", SHAPE_NAME.get(int(shape_id), f"Shape {shape_id}"))
    ...
    print("checkpoints_visited:", f"{visited_count}/{total_count}")
    visit_fraction = info_final.get("visit_fraction", torch.tensor([0.0], device=env_u.device))
    print("visit_fraction:", float(torch.as_tensor(visit_fraction).reshape(-1)[0].detach().cpu().item()))
