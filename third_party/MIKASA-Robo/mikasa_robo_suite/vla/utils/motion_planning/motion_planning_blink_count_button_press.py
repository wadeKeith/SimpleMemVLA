...
def parse_args():
    ...
    parser.add_argument(
        "--env-id",
        default="BlinkCountButtonPressMedium-VLA-v0",
        help="Registered BlinkCountButtonPress-* env id.",
    )
...
def main():
    ...
    if not any(env_id.startswith(p) for p in VALID_ENV_PREFIXES):
        raise ValueError(f"Expected a BlinkCountButtonPress-* env, got {env_id!r}.")
    ...
    z_press = button_top_z + float(env_u.BUTTON_PRESS_Z_MARGIN) - T - EXTRA_PRESS_DEPTH
    z_unpress = z_press + 0.05
    z_approach = z_unpress + 0.10
    z_lift = button_top_z + float(env_u.REQUIRED_LIFT_HEIGHT) + 0.02
    z_confirm_press = confirm_button_top_z + float(env_u.BUTTON_PRESS_Z_MARGIN) - T - EXTRA_PRESS_DEPTH
    z_confirm_unpress = z_confirm_press + 0.05
    z_confirm_approach = z_confirm_unpress + 0.10
    z_confirm_lift = confirm_button_top_z + float(env_u.REQUIRED_LIFT_HEIGHT) + 0.02
    ...
    for _ in range(target_blinks):
        ...
        target_lift = sapien.Pose(p=[button_xy[0], button_xy[1], z_lift], q=tcp_q)
