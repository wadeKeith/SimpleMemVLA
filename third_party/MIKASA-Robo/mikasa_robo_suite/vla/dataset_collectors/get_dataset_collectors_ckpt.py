...
class Args:
    ...
    """controller for train AND eval envs. "pd_ee_delta_pose" is the official
    VLA protocol; "pd_joint_pos" trains a joint-space oracle for the joint
    dataset variant."""
    override_gamma: Optional[float] = None
    """if set, wins over the per-env ENVS_CONFIG gamma (which was tuned for
    pd_ee_delta_pose; joint-delta oracles need longer credit horizons)."""
    override_gae_lambda: Optional[float] = None
    """if set, wins over the per-env ENVS_CONFIG gae_lambda."""
    override_ent_coef: Optional[float] = None
    """if set, wins over the default/per-env entropy coefficient."""
    override_total_timesteps: Optional[int] = None
    """if set, wins over the hardcoded 150M env-step budget."""
    eval_found_lost_pairs_capacity: Optional[int] = None
    """override PhysX found_lost_pairs_capacity for the eval envs only. Some
    driver/GPU combos crash in px.gpu_init() when a second PhysX system in the
    same process requests the env's full default pool (seen with
    RememberColor9-VLA-v0); a smaller pool for the 16 eval envs avoids it
    without touching the training envs or physics behavior."""
...
if __name__ == "__main__":
    ...
    # Explicit CLI overrides win over ENVS_CONFIG / the hardcoded budget
    # (ENVS_CONFIG hyper-params were tuned for pd_ee_delta_pose).
    if args.override_gamma is not None:
        args.gamma = args.override_gamma
    if args.override_gae_lambda is not None:
        args.gae_lambda = args.override_gae_lambda
    if args.override_ent_coef is not None:
        args.ent_coef = args.override_ent_coef
    if args.override_total_timesteps is not None:
        args.total_timesteps = args.override_total_timesteps
    ...
    if args.eval_found_lost_pairs_capacity is not None:
        eval_env_kwargs["sim_config"] = dict(
            gpu_memory_config=dict(found_lost_pairs_capacity=args.eval_found_lost_pairs_capacity)
        )
    eval_envs = gym.make(
        args.env_id, num_envs=args.num_eval_envs, reconfiguration_freq=args.eval_reconfiguration_freq, **eval_env_kwargs
