...
class FindImposterShapeAndColorVLABaseEnv(BaseEnv):
    """Find the object whose (shape, color) combination was NOT in the first phase.

    Episode flow:
    - Phase 1 (cue):  SHAPES-1 objects are shown at spread positions.
    ...
    def _apply_phase_layout(self, elapsed_steps: torch.Tensor, env_mask: torch.Tensor = None) -> None:
        ...
        for key in self.shape_color_dict:
            ...
            if has_hidden:
                ...
                is_imposter = self.true_shape_indices == key
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # imposter_key: which combo is hidden in cue but appears in manip
            ...
            # Spread radius: larger for more objects to keep adequate spacing
            radius = 0.3 if n <= 9 else 0.45
            for key in self.shape_color_dict:
...
@register_env("FindImposterShapeAndColor3x2-VLA-v0", max_episode_steps=25)
class FindImposterShapeAndColor3x2VLAEnv(FindImposterShapeAndColorVLABaseEnv):
    SHAPES = 3 * 2
    CUE_PHASE_STEPS = [1, 5]
    EMPTY_PHASE_STEPS = [1, 5]


@register_env("FindImposterShapeAndColor3x3-VLA-v0", max_episode_steps=25)
class FindImposterShapeAndColor3x3VLAEnv(FindImposterShapeAndColorVLABaseEnv):
    SHAPES = 3 * 3
    CUE_PHASE_STEPS = [1, 5]
    EMPTY_PHASE_STEPS = [1, 5]


@register_env("FindImposterShapeAndColor5x3-VLA-v0", max_episode_steps=40)
class FindImposterShapeAndColor5x3VLAEnv(FindImposterShapeAndColorVLABaseEnv):
    SHAPES = 5 * 3
    CUE_PHASE_STEPS = [1, 5]
    EMPTY_PHASE_STEPS = [1, 5]
