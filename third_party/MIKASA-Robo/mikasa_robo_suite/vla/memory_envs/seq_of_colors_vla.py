...
class SeqOfColorsVLABaseEnv(BaseEnv):
    """Remember a color sequence, but only recover the set of its items.

    The cue is presented as an ordered sequence, but during manipulation the
    robot is allowed to touch the target colors in any order. This makes the
    task easier than `ChainOfColors`, because the agent must remember membership
    but not replay the temporal order.

    Episode flow:
    - A sequence of cue colors is shown one by one.
    - All cubes disappear during the memory phase.
    - All cubes reappear and the robot selects the target subset.

    Success (`success=True`):
    - The robot must touch every color that appeared in the cue sequence and
      avoid touching colors that were not part of that sequence.

    How to customize:
    - `SEQUENCE_LENGTH` changes how many cue items become targets.
    - `COLORS` changes how many total candidate colors exist.
    - `CUE_STEP_DURATION` changes the visibility duration of each cue item.
    - `EMPTY_PHASE_STEPS` changes the length of the memory delay.
    - `GOAL_THRESH` changes how close the robot must get for a touch to count.
    - `CUBE_HALFSIZE` changes cube size and spacing.
    """
    ...
    def evaluate(self):
        ...
        touched_positions = self.touched_cubes_idx != -1
        wrong_order_touched = (
            touched_positions & (self.touched_cubes_idx != self.true_color_indices.to(dtype=torch.long))
        ).any(1)
        self.wrong_order_touched = wrong_order_touched
        ...
        empty_positions = self.touched_cubes_idx == -1
        next_touch_pos = torch.argmax(empty_positions.float(), dim=1)
        has_remaining_targets = empty_positions.any(dim=1) & (~wrong_order_touched)
        batch_indices = torch.arange(sequence_cubes_mask.shape[0], device=self.device)
        target_cube_indices = self.true_color_indices[batch_indices, next_touch_pos].to(dtype=torch.long)
        next_target_mask[batch_indices, target_cube_indices] = has_remaining_targets
...
# ----- Standard tasks -----
@register_env("SeqOfColors3-VLA-v0", max_episode_steps=400)
class SeqOfColors3VLAEnv(SeqOfColorsVLABaseEnv):
    SEQUENCE_LENGTH = 3
    CUE_STEP_DURATION = [1, 5]
    EMPTY_PHASE_STEPS = [1, 5]


@register_env("SeqOfColors5-VLA-v0", max_episode_steps=400)
class SeqOfColors5VLAEnv(SeqOfColorsVLABaseEnv):
    SEQUENCE_LENGTH = 5
    CUE_STEP_DURATION = [1, 5]
    EMPTY_PHASE_STEPS = [1, 5]


@register_env("SeqOfColors7-VLA-v0", max_episode_steps=400)
class SeqOfColors7VLAEnv(SeqOfColorsVLABaseEnv):
    SEQUENCE_LENGTH = 7
    CUE_STEP_DURATION = [1, 5]
    EMPTY_PHASE_STEPS = [1, 5]
...
# ----- Long-horizon tasks -----
@register_env("SeqOfColors3-Long-VLA-v0", max_episode_steps=800)
class SeqOfColors3LongVLAEnv(SeqOfColorsVLABaseEnv):
    SEQUENCE_LENGTH = 3
    CUE_STEP_DURATION = [10, 100]
    EMPTY_PHASE_STEPS = [50, 400]


@register_env("SeqOfColors5-Long-VLA-v0", max_episode_steps=1000)
class SeqOfColors5LongVLAEnv(SeqOfColorsVLABaseEnv):
    SEQUENCE_LENGTH = 5
    CUE_STEP_DURATION = [10, 100]
    EMPTY_PHASE_STEPS = [50, 400]


@register_env("SeqOfColors7-Long-VLA-v0", max_episode_steps=1200)
class SeqOfColors7LongVLAEnv(SeqOfColorsVLABaseEnv):
    SEQUENCE_LENGTH = 7
    CUE_STEP_DURATION = [10, 100]
    EMPTY_PHASE_STEPS = [50, 400]
