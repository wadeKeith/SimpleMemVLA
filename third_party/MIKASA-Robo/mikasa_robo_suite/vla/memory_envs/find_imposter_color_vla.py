...
class FindImposterColorVLABaseEnv(BaseEnv):
    """Find the cube whose color was NOT present in the first phase.

    Episode flow:
    - Phase 1 (cue):  COLORS-1 cubes are shown at spread positions.
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # imposter_key[env_i]: which color is hidden in cue but appears in manip
            self.imposter_key = self._batched_episode_rng.choice(list(self.color_dict.keys()))
    ...
    def evaluate(self):
        ...
        # Goal: reach the imposter cube
        self.masks = {key: (self.imposter_key == key).unsqueeze(-1) for key in self.color_dict}
...
@register_env("FindImposterColor3-VLA-v0", max_episode_steps=25)
class FindImposterColor3VLAEnv(FindImposterColorVLABaseEnv):
    COLORS = 3
    CUE_PHASE_STEPS = [1, 5]
    EMPTY_PHASE_STEPS = [1, 5]


@register_env("FindImposterColor5-VLA-v0", max_episode_steps=25)
class FindImposterColor5VLAEnv(FindImposterColorVLABaseEnv):
    COLORS = 5
    CUE_PHASE_STEPS = [1, 5]
    EMPTY_PHASE_STEPS = [1, 5]


@register_env("FindImposterColor9-VLA-v0", max_episode_steps=25)
class FindImposterColor9VLAEnv(FindImposterColorVLABaseEnv):
    COLORS = 9
    CUE_PHASE_STEPS = [1, 5]
    EMPTY_PHASE_STEPS = [1, 5]
