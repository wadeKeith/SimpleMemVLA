...
class BunchOfColorsEnv(BaseEnv):
    ...
    def evaluate(self):
        ...
        empty_table = (self.elapsed_steps >= self.STEP_DURATION) & (
            self.elapsed_steps < self.STEP_DURATION + self.EMPTY_DURATION
        )
        ...
        offsets = torch.stack(
            [radius * cos_angles, radius * sin_angles, torch.zeros_like(keys, device=self.device)], dim=1
        )  # Shape: [num_cubes, 3]
