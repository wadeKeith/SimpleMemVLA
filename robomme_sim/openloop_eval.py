...
from simplememvla.benchmarks.robomme import NUM_ACTIONS_CHUNK


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("--pretrained_checkpoint", required=True)
    parser.add_argument("--root", default="./data/datasets/yinchenghust/robomme_lerobot")
    parser.add_argument("--repo_id", default="yinchenghust/robomme_lerobot")
    ...
    parser.add_argument("--max_subtask_tokens", type=int, default=64)
    parser.add_argument(
        "--num_open_loop_steps", type=int, default=16,
        help="Re-predict every N EXECUTION frames (receding horizon). Defaults to 16 to "
             "match the closed-loop eval's --execute_horizon, so this open-loop score is "
             "a faithful quality proxy for closed loop (same re-decision cadence).",
    )
    parser.add_argument(
        "--include_video_demo_anchors", action="store_true",
        help="Also predict at is_video_demo frames (subtask='static', action=hold). "
             "Default off: the deployment path never queries the policy there.",
    )
...
def _is_video_demo(item) -> bool:
    flag = item.get("is_video_demo")
    if flag is None:
        return False
...
def main() -> None:
    ...
    sub_key = str(getattr(vla.config, "subtask_index_key", "subtask_online_index"))
    if meta.subtasks is None:
        raise RuntimeError(
            f"subtask_index_key={sub_key!r} needs meta/subtasks.parquet"
        )
    _subtask_by_index = {
        int(v): s for s, v in meta.subtasks["subtask_index"].items()
    }

    def gt_subtask(item):
        return _subtask_by_index[int(item[sub_key].reshape(-1)[0])]

    print(f"Scoring sub-task accuracy against column: {sub_key}")
    ...
    tot_l1, tot_steps, tot_sub_hit, tot_sub = 0.0, 0, 0, 0
    for ep in episodes:
        ds = LeRobotDataset(
        ...
        for i in range(len(ds)):
            ...
            if not args.include_video_demo_anchors and _is_video_demo(item):
                continue
            anchor = exec_i % args.num_open_loop_steps == 0
            exec_i += 1
            if not anchor:
                continue
            ...
            ep_hit += int(_norm(gen_subtask) == _norm(gt_subtask(item)))
            ep_pred += 1
        l1 = ep_l1 / max(ep_steps, 1)
        acc = ep_hit / max(ep_pred, 1)
        print(f"  ep {ep:>3}: action_L1={l1:.4f}  subtask_acc={acc:.3f} "
    ...
    print("=" * 60)
    print(f"Overall action_L1 : {tot_l1 / max(tot_steps, 1):.4f} (raw joint space)")
    print(f"Overall subtask_acc: {tot_sub_hit / max(tot_sub, 1):.3f} "
          f"({tot_sub_hit}/{tot_sub})")
